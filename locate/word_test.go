...
func TestNextWordStartInLine(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			textTree, err := text.NewTreeFromString(tc.inputString)
			require.NoError(t, err)
			actualPos := NextWordStartInLine(textTree, tc.pos, tc.count)
			assert.Equal(t, tc.expectedPos, actualPos)
		})
	}
}
...
func TestNextWordStartInLineOrAfterEmptyLine(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			textTree, err := text.NewTreeFromString(tc.inputString)
			require.NoError(t, err)
			actualPos := NextWordStartInLineOrAfterEmptyLine(textTree, tc.pos, tc.count)
			assert.Equal(t, tc.expectedPos, actualPos)
		})
	}
}
...
func TestCurrentWordEndWithTrailingWhitespace(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			textTree, err := text.NewTreeFromString(tc.inputString)
			require.NoError(t, err)
			actualPos := CurrentWordEndWithTrailingWhitespace(textTree, tc.pos, tc.count)
			assert.Equal(t, tc.expectedPos, actualPos)
		})
	}
}
